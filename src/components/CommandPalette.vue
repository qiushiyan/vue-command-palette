...
const commandListFiltered = computed(() => {
    if (commandFilterText.value === "") {
        return commandStore.commandList
    }

    return commandStore.commandList.filter((command) => {
        return command.title.toLowerCase().includes(commandFilterText.value.toLowerCase())
    })
})
...
<template>
    <Teleport to="body">
        <div class="w-6/12 m-auto" v-if="commandStore.showCommandPalette">
            ...
            <div class="relative w-full">
                ...
                <input type="text" id="search" v-model="commandFilterText"
